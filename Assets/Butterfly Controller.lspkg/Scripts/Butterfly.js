...
script.thisButterflyIndex; // the index of this butterfly, could be useful for making some butterflies behave differently

script.start = function (mainController) {
...
function startFlying() {
	...
	allClassificationsAllowed = controller.landOnClassifications.length === 7; // do not check each index if all are allowed
...
function setPersonality() {
	...
	var directionalSpeed = controller.directionalSpeed;
...
function startMoving() {
	...
	function moveUpdate() {
		...
		var isWorldTracking = controller.landOnWorldMesh && controller.deviceTrackingComponent;
		...
		var movingSpeed = controller.movingSpeed;
		...
		var followTransform = controller.followTransform;
		var followStrength = controller.followStrength;
		...
		if (isWorldTracking && controller.avoidCollision) newPos = collisionCorrected(oldPos, newPos); // skip territory following when collision is detected
...
function collisionCorrected(oldPos, newPos) {
	...
	var hitArray = controller.deviceTrackingComponent.raycastWorldMesh(oldPos, newPos);
...
function worldMeshCheck() {
	if (!trackingInitialized && controller.thisButterflyIndex === 0) { // on the first butterfly, check if the Tracking Component is initialized
		controller.deviceTrackingComponent.worldOptions.enableWorldMeshesTracking = true;
		if (!allClassificationsAllowed) controller.deviceTrackingComponent.worldOptions.enableWorldMeshesClassificationTracking = true;
	...
	function doWorldMeshCheck() {
		...
		var hitArray = controller.deviceTrackingComponent.raycastWorldMesh(thisPos, thisPos.add(raycastVector));
		...
		if (hitArray.length > 0) {
			...
			if (hitDistance < worldMeshRaycastDistance) {// if within max search distance
				if (!allClassificationsAllowed) {
					if (controller.landOnClassifications.indexOf(hit.classification) === -1) { // if not one of specified classifications
						return;
					}
				}
			} else {
...
function restingFlappingAnimation() {
	...
	function stayInPlace() {
		...
		if (!controller.landOnWorldMesh) startFlying();
	}
...
function isInTerritory(pos) {
	...
	var size = controller.followRadius;

	if(controller.alwaysAboveGround && pos.y < 0) return false;
